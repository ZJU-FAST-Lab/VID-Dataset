...
bool DjiRos::initPublisher(ros::NodeHandle &nh) {
  ...
  imuwithhwts_publisher = nh.advertise<djiros::ImuWithHwTimeStamp>("imu_hwts", 10);
  pulse_publisher = nh.advertise<djiros::Pulse>("pulse", 10);
...
void DjiRos::onReceive400HzData(Vehicle *vehicle,
  ...
  // if(hs_data.ts.flag == 1)
  // {
  //   printf("index = %d   ", hs_data.ts.index);
  //   printf("flag = %d   ", hs_data.ts.flag);
  //   printf("time2p5ms = %u   ", hs_data.ts.time2p5ms);
  //   printf("time1ns = %10u", hs_data.ts.time1ns);
  //   printf("\r\n");
  // }
  ...
  djiros::ImuWithHwTimeStamp imuhwts_msg;
  imuhwts_msg.header.stamp = imu_msg.header.stamp;
  imuhwts_msg.header.frame_id = imu_msg.header.frame_id;
  imuhwts_msg.orientation = imu_msg.orientation;
  imuhwts_msg.angular_velocity = imu_msg.angular_velocity;
  imuhwts_msg.linear_acceleration = imu_msg.linear_acceleration;

  imuhwts_msg.index = hs_data.ts.index;
  imuhwts_msg.flag = hs_data.ts.flag;

  imuhwts_msg.hwts.sec = floor ( (float)hs_data.ts.time2p5ms / 400.0 );
  imuhwts_msg.hwts.nsec = (hs_data.ts.time2p5ms % 400) * 2500000;

  p->imuwithhwts_publisher.publish(imuhwts_msg);



  djiros::Pulse pulse_msg;
  pulse_msg.header.frame_id = imu_msg.header.frame_id;
  pulse_msg.header.stamp = imu_msg.header.stamp;
  pulse_msg.hwts = imuhwts_msg.hwts;

  if(hs_data.ts.flag == 1)
  {
    p->pulse_publisher.publish(pulse_msg);
  }
}
